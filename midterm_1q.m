function midterm_1q(x)

path=strcat('t:\xfer\xfer_res\jemele\projects\ucsd\wes265\assignments\midterm\1q')
...
h_s = filter(h,1,x)
h_m = conv(h,h_s)/(h*h')
interference = 0.2*exp(j*2*pi*(0:length(x)-1)*0.06)
h_s_n = h_s + interference'
...
grid on
axis([-2 2 -2 2])
title(['Constellation Diagram, Interference + Matched Filter, \alpha=',num2str(alpha)])
